...
void OnetKernel::raw_error()
{
    ...
    pTabC->showMessageAll(strMessage, InfoMessage);
}
...
void OnetKernel::raw_part()
{
    ...
    pTabC->showMessage(strChannel, strDisplay, PartMessage);
...
void OnetKernel::raw_kick()
{
    ...
    pTabC->showMessage(strChannel, strDisplay, KickMessage);
...
void OnetKernel::raw_mode()
{
    ...
    if ((strNickOrChannel[0] == '#') || (strNickOrChannel[0] == '^'))
    {
        ...
        while (i.hasPrevious())
        {
            ...
            pTabC->showMessage(strChannel, strDisplay, ModeMessage);
...
void OnetKernel::raw_notice()
{
    ...
    if (strNick[0] == '#')
        pTabC->showMessage(strNick, strDisplay, NoticeMessage);
    else
...
void OnetKernel::raw_invreject()
{
    ...
    pTabC->showMessage(strChannel, strDisplay, InfoMessage);
}
...
void OnetKernel::raw_modermsg()
{
    ...
    QString strDisplay = QString("%1 [%2 %3]").arg(strMessage).arg(tr("Moderated by")).arg(strModerator);

    // display
    pTabC->showMessage(strChannel, strDisplay, DefaultMessage, QString::null, strNick);
}
...
void OnetKernel::raw_kill()
{
    ...
    // display
    pTabC->showMessageAll(strDisplay, ErrorMessage);
}
...
void OnetKernel::raw_109n()
{
    ...
    // display
    pTabC->showMessage(strChannel, strMessage, NoticeMessage);
}
...
void OnetKernel::raw_251n()
{
    ...
    if (strNick.toLower() == "chanserv")
    {
        ...
        pTabC->showMessageActive(strDisplay, ModeMessage);
    }
    else if (strNick.toLower() == "nickserv")
    {
        QString strNick = strDataList[4];
...
void OnetKernel::raw_311()
{
    ...
    pTabC->showMessageActive(strDisplayNick, InfoMessage);

    QString strDisplayIrcname = QString(tr("* %1 ircname: %2")).arg(strNick).arg(strIrcname);
    pTabC->showMessageActive(strDisplayIrcname, InfoMessage);
}
...
void OnetKernel::raw_802()
{
    ...
    pTabC->showMessageActive(strMessage, ErrorMessage);
}
...
void OnetKernel::raw_817()
{
    ...
    if (!strMessage.isEmpty())
        pTabC->showMessage(strChannel, strMessage, DefaultMessage, strTime, strNick);
}
